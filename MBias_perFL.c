...
void *extractMBiasFL(void *foo) {
    Config *config = (Config*) foo;
    bam_hdr_t *hdr;
    bam_mplp_t iter;
    int ret, tid, pos = 0, i, j, seqlen, rv, o = 0, modified_position, isizeInd;
    int32_t bedIdx = 0;
    int strand;
    ...
    int isizeRange = config->maxIsize - config->minIsize + 1;
    strandMeth ***meths = malloc(isizeRange*sizeof(strandMeth**));
    assert(meths);
    ...
    for (i=0; i<isizeRange; i++) {
        meths[i] = malloc(4*sizeof(strandMeth*));
        for(j=0; j<4; j++) {
            meths[i][j] = calloc(1, sizeof(strandMeth));
            assert(meths[i][j]);
        }
    }
        ...
        while((ret = bam_mplp_auto(iter, &tid, &pos, &n_plp, plp)) > 0) {
            ...
            for(i=0; i<n_plp; i++) {
                ...
                if(rv != 0) {
                    ...
                    isizeInd = abs((plp[0]+i)->b->core.isize) - config->minIsize;
                    if(modified_position >= meths[isizeInd][strand-1]->m)
                        meths[isizeInd][strand-1] = growStrandMeth(meths[isizeInd][strand-1], modified_position);
                    if(rv < 0) {
                        if((plp[0]+i)->b->core.flag & BAM_FREAD2) {
                            assert((meths[isizeInd][strand-1]->unmeth2[modified_position]) < 0xFFFFFFFF);
                            meths[isizeInd][strand-1]->unmeth2[modified_position]++;
                        } else {
                            assert((meths[isizeInd][strand-1]->unmeth1[modified_position]) < 0xFFFFFFFF);
                            meths[isizeInd][strand-1]->unmeth1[modified_position]++;
                        }
                    } else {
                        if((plp[0]+i)->b->core.flag & BAM_FREAD2) {
                            assert((meths[isizeInd][strand-1]->meth2[modified_position]) < 0xFFFFFFFF);
                            meths[isizeInd][strand-1]->meth2[modified_position]++;
                        } else {
                            assert((meths[isizeInd][strand-1]->meth1[modified_position]) < 0xFFFFFFFF);
                            meths[isizeInd][strand-1]->meth1[modified_position]++;
                        }
                    }
                    if(modified_position+1 > meths[isizeInd][strand-1]->l) meths[isizeInd][strand-1]->l = modified_position+1;
                }
            }
        }
...
void makeTXT_perFL(strandMeth **m, int isize, int header) {
    char *abbrevs[4] = {"OT", "OB", "CTOT", "CTOB"};
    int i, j;

    if(header) printf("Strand\tRead\tPosition\tnMethylated\tnUnmethylated\tiSize\n");
    for(i=0; i<4; i++) {
        if(m[i]->l) {
            for(j=0; j<m[i]->l; j++) {
                if(m[i]->meth1[j] || m[i]->unmeth1[j])
                    printf("%s\t1\t%i\t%"PRIu32"\t%"PRIu32"\t%i\n", abbrevs[i], j+1, m[i]->meth1[j], m[i]->unmeth1[j], isize);
                if(m[i]->meth2[j] || m[i]->unmeth2[j])
                    printf("%s\t2\t%i\t%"PRIu32"\t%"PRIu32"\t%i\n", abbrevs[i], j+1, m[i]->meth2[j], m[i]->unmeth2[j], isize);
            }
        }
    }
}
...
"                  applications such as cell-free DNA. Inclusive. default=1 for mbias \n"
" --maxIsize INT   Maximum insert size / fragment length. Also inclusive. default=500 for mbias \n"
...
"                  This and noSVG has become the default.\n"
" --noSVG          SVG would not work in this modified mbias function. By default noSVG.\n"
...
int mbiasFL_main(int argc, char *argv[]) {
    // char *opref = NULL;
    int c, i, j, k, SVG = 0, txt = 1, keepStrand = 0;
    ...
    if(config.maxIsize < 0) {
        fprintf(stderr, "--maxIsize %i is invalid. Resetting to 500, which is the default value.\n", config.maxIsize);
        config.maxIsize = 500;
    }
    ...
    for(i=0; i < config.nThreads; i++) pthread_create(threads+i, NULL, &extractMBiasFL, &config);
    for(i=0; i < config.nThreads; i++) {
        pthread_join(threads[i], (void**) &threadout);
        for (j=0; j<isizeRange; j++) {
            for(k=0; k<4; k++) {
                meths[j][k] = mergeStrandMeth(meths[j][k], threadout[j][k]);
                if(threadout[j][k]->meth1) free(threadout[j][k]->meth1);
                if(threadout[j][k]->unmeth1) free(threadout[j][k]->unmeth1);
                if(threadout[j][k]->meth2) free(threadout[j][k]->meth2);
                if(threadout[j][k]->unmeth2) free(threadout[j][k]->unmeth2);
                free(threadout[j][k]);
            }
            free(threadout[j]);
        }
    ...
    for (i=0; i<isizeRange; i++) {
        for(j=0; j<4; j++) {
            if(meths[i][j]->meth1) free(meths[i][j]->meth1);
            if(meths[i][j]->unmeth1) free(meths[i][j]->unmeth1);
            if(meths[i][j]->meth2) free(meths[i][j]->meth2);
            if(meths[i][j]->unmeth2) free(meths[i][j]->unmeth2);
            free(meths[i][j]);
        }
        free(meths[i]);
    }
